...
@implementation FeedFourImageNewsCell {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if(self) {
        ...
        photoFourView = [[NSBundle.mainBundle loadNibNamed:@"PhotoFourView" owner:self options:nil] objectAtIndex:0];
        [self addSubview: photoFourView];
    }
    return self;
}
...
-(void) fillData:(News *)news {
    ...
    float yFooterView = yPhotoView + constant.spacing + photoFourView.heightView;
    [self addSubview: photoFourView];
    [photoFourView setFrame:CGRectMake(constant.margin, yPhotoView,constant.maxWidth, photoFourView.heightView)];
    [photoFourView fillData:news];
